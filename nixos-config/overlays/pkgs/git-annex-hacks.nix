...

{
  ...
  git-annex = self.callPackage (

    { git, gitAnnex, writeScriptBin, symlinkJoin }:
    ...
    symlinkJoin {
      ...
      paths = [ git-annex-bin gitAnnex ];
    }

  ) { inherit (self.gitAndTools) gitAnnex; };





  git-annex-desktop = self.callPackage (

    { stdenv, fetchurl, writeScript, writeText }:
